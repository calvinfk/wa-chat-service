...
func (u *StorageMediaUsecase) UploadMedia(ctx context.Context, tenantID string, inputData dto.StorageMediaUploadRequest) (dto.StorageMediaResponse, bool, error) {
	...
	var whatsappClient *whatsapp_business.Client
	if inputData.PhoneNumberId != nil {
	...
	if inputData.SaveMeta != nil && *inputData.SaveMeta {
		whatsappClient, _, err := u.waBusinessAccountUsecase.GetWhatsappClient(ctx, tenantID, *inputData.PhoneNumberId)
		if err != nil {
			u.zsLog.Errorf("[UploadMedia] Failed to get tenant data from repository: %v", err)
			if err == errs.ErrGenericNotFound {
	...
	// TODO: resumable returns con closed
	if inputData.SaveResumable != nil && *inputData.SaveResumable {
	...
	if inputData.SaveStorage != nil && *inputData.SaveStorage {
...
func (u *StorageMediaUsecase) getMediaStreamByID(ctx context.Context, storageMediaID string, rangeHeader string) (dto.StorageMediaGetMediaResponse, bool, error) {
	...
	} else if media.MediaId != nil && media.PhoneNumberId != nil {
		// if media ID exists, get the media URL from WhatsApp Business API and stream it
		whatsappClient, _, err := u.waBusinessAccountUsecase.GetWhatsappClient(ctx, "", *media.PhoneNumberId)
		if err != nil {
...
func (u *StorageMediaUsecase) DeleteMedia(ctx context.Context, tenantID string, inputData dto.StorageMediaDeleteRequest) (bool, error) {
	...
	if media.MediaId != nil {
		whatsappClient, _, err := u.waBusinessAccountUsecase.GetWhatsappClient(ctx, tenantID, *media.PhoneNumberId)
		if err != nil {
...
func (u *StorageMediaUsecase) SaveMediaID(ctx context.Context, tenantID string, inputData dto.StorageMediaSaveMediaIDRequest) (dto.StorageMediaSaveMediaIDResponse, bool, error) {
...
func (u *StorageMediaUsecase) GetFilteredByTenantID(ctx context.Context, tenantID string, inputData filter_request.FilterRequest[dto.StorageMediaGetListRequest]) (filter_request.FilterResponse[dto.StorageMediaResponse], bool, error) {
	var response filter_request.FilterResponse[dto.StorageMediaResponse]
	data, paginate, totalItems, err := u.storageMediaRepository.GetFilteredByTenantID(ctx, tenantID, inputData)
	if err != nil {
