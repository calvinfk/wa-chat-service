	...
	for _, component := range sendComponents {
		...
		for _, p := range componentParametersAny {
			if param, ok := p.(map[string]any); ok {
				...
				return fmt.Errorf("invalid parameter format in the payload for component type %s, expected array of objects", componentType)
			}
		}
		...
		case "HEADER":
			for _, p := range componentParameters {
				...
				if err != nil {
					return fmt.Errorf("type field is required for parameters in %s but missing or not a string", componentType)
				}
				...
				if componentParameterType == "text" {
					...
					if err != nil {
						return fmt.Errorf("%s field is required for parameters in %s but missing or invalid", componentParameterType, componentType)
					}
				}

				_, err = whatsapp_business.NewComponent(componentParameterType, componentParameterPayload)
				if err != nil {
					return fmt.Errorf("failed to parse component for type %s.%s: %v", componentType, componentParameterType, err)
				}

				if whatsapp_business.IsMediaMessageType(componentParameterType) {
					mediaID, ok := componentParameterPayload["id"].(string)
					if !ok {
						return fmt.Errorf("media id is required for media component in %s but missing or not a string", componentType)
					}
